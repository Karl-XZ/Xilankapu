...
import { applyFavoriteToCandidates, checkGenerationStatus, submitGeneration, subscribeGenerationUpdates } from './services/api';
import type {
  AppTab,
  AuthSession,
  CanvasView,
...
const GRID_RESOLUTION_PRESETS = [16, 24, 32, 48, 64, 96, 128, 160] as const;
const MIN_GRID_RESOLUTION = 16;
const MAX_GRID_RESOLUTION = 160;

function normalizeGridResolution(value: number) {
  if (!Number.isFinite(value)) {
    return DEFAULT_WORKSPACE_STATE.gridDensity;
  }

  return Math.min(MAX_GRID_RESOLUTION, Math.max(MIN_GRID_RESOLUTION, Math.round(value)));
}

function gridResolutionText(value: number) {
  return `${value} x ${value}`;
}

function getPreviewCellSize(size: number) {
  return Math.max(4, Math.floor(320 / Math.max(size, 1)));
}

function getEditorCellMinSize(size: number) {
  if (size >= 160) {
    return 4;
  }
...
function normalizeWorkspaceState(workspace: WorkspaceState): WorkspaceState {
  return {
    ...workspace,
    gridDensity: normalizeGridResolution(workspace.gridDensity),
  };
}
...
function App() {
  ...
  const [workspace, setWorkspace] = useState<WorkspaceState>(() => normalizeWorkspaceState(DEFAULT_WORKSPACE_STATE));
  const [gridResolutionInput, setGridResolutionInput] = useState(() =>
    String(normalizeGridResolution(DEFAULT_WORKSPACE_STATE.gridDensity)),
  );
  ...
  useEffect(() => {
    setGridResolutionInput(String(workspace.gridDensity));
  ...
  useEffect(() => {
    ...
    return subscribeGenerationUpdates(activeGenerationId, (nextTask) => {
      setGenerationTask(nextTask);
    });
  }, [activeGenerationId]);
  ...
  const previewSvg = useMemo(() => {
    ...
    return matrixToSvg(matrix, {
      ...
      cellSize: getPreviewCellSize(matrix.size),
      padding: 24,
    });
  ...
  const gridResolutionPresetValue = GRID_RESOLUTION_PRESETS.includes(
    workspace.gridDensity as (typeof GRID_RESOLUTION_PRESETS)[number],
  )
    ? String(workspace.gridDensity)
    : 'custom';

  const updateWorkspace = <K extends keyof WorkspaceState>(key: K, value: WorkspaceState[K]) => {
    const nextValue =
      key === 'gridDensity'
        ? (normalizeGridResolution(value as number) as WorkspaceState[K])
        : value;

    setWorkspace((previous) => ({ ...previous, [key]: nextValue }));
  };
  ...
  const handleGridResolutionPresetChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    const nextResolution = normalizeGridResolution(Number.parseInt(value, 10));
    setGridResolutionInput(String(nextResolution));
    updateWorkspace('gridDensity', nextResolution);
  };

  const handleGridResolutionInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    setGridResolutionInput(event.target.value.replace(/[^\d]/g, ''));
  };

  const commitGridResolutionInput = (rawValue = gridResolutionInput) => {
    ...
    const nextResolution = normalizeGridResolution(Number.parseInt(digitsOnly, 10));
    setGridResolutionInput(String(nextResolution));
    updateWorkspace('gridDensity', nextResolution);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setStatusMessage(
        uploadedFiles.length > 0
          ? `生成任务已提交，已传入 ${uploadedFiles.length} 张参考图，正在轮询候选结果…`
          : '生成任务已提交，正在轮询候选结果…',
      );
  ...
  const loadProject = (project: SavedProject) => {
    const mergedWorkspace = normalizeWorkspaceState({
      ...DEFAULT_WORKSPACE_STATE,
      ...project.workspace,
    });
    const reuseSavedMatrix = project.matrix?.size === mergedWorkspace.gridDensity;
    setWorkspace(mergedWorkspace);
    setLoadedProjectId(project.id);
    setProjectFeedback(`已载入项目：${project.name}`);
    setMatrix(reuseSavedMatrix ? project.matrix : null);
    setPathPlan(reuseSavedMatrix ? project.pathPlan : null);
    ...
    setMatrixFingerprint(
      reuseSavedMatrix ? buildMatrixFingerprint(project.selectedCandidate?.id || null, mergedWorkspace) : '',
    );
    setActiveTab('workspace');
  };
      ...
      return (
        <div className="editor-stage">
          <div
            ...
            style={{ gridTemplateColumns: `repeat(${matrix.size}, minmax(${getEditorCellMinSize(matrix.size)}px, 1fr))` }}
          >
          ...
          <section className="workspace-section">
            <aside className="workspace-sidebar">
              ...
              <div className="sidebar-section">
                ...
                <div className="two-column-row">
                  <div className="param-group">
                    <label className="param-label">网格分辨率</label>
                    <select
                      className="param-select"
                      value={gridResolutionPresetValue}
                      onChange={handleGridResolutionPresetChange}
                    >
                    ...
                    <span className="field-label grid-resolution-caption">{'\u81ea\u5b9a\u4e49\u8fb9\u957f'}</span>
                    <div className="grid-resolution-input-row">
                      <input
                        className="param-input"
                        inputMode="numeric"
                        pattern="[0-9]*"
                        placeholder={'\u8f93\u5165\u6b63\u65b9\u5f62\u8fb9\u957f'}
                        value={gridResolutionInput}
                        onChange={handleGridResolutionInputChange}
                        onBlur={() => commitGridResolutionInput()}
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                      ...
                      />
                      <span className="grid-resolution-badge">{gridResolutionText(workspace.gridDensity)}</span>
                    </div>
              ...
              <div className="sidebar-section">
                ...
                <div className="upload-preview-list">
                  ...
                  {uploadedPreviews.length > 0 && (
                    <p className="form-hint success-text">本次生成会把以上 {uploadedPreviews.length} 张参考图作为图生图输入一并传给 API。</p>
                  )}
